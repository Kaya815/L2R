...
def pre_allocate_batch(dict_univ_bin, num_docs_per_batch):
    ...
    if 1 == num_docs_per_batch: # a simple but time-consuming per-query processing, namely the batch_size is always one
        ...
        for univ in dict_univ_bin:
            bin = dict_univ_bin[univ]#univ: documents number of query
            bin_length = len(bin)#length of indexs of queries
...
class TabNetRanker():
    ...
    def __init__(self, id='TabNetRanker', gpu=False, device=None):
        self.id = id
    ...
    def init(self):
        ...
        self.reducing_matrix = create_explain_matrix(
            ...
            self.sf.cat_emb_dim,
            self.sf.cat_idxs,
            self.sf.post_embed_dim,
        )
        self._update_network_params()
        #for item in self.sf.parameters():
            #print(item)
        self.optimizer = optim.Adam(self.sf.parameters(), lr=0.02, weight_decay=0.001)




    def _update_network_params(self):
        self.sf.virtual_batch_size = self.virtual_batch_size
    def eval_mode(self):
        '''
        ...
        For example, Dropouts Layers, BatchNorm Layers etc. Yon_steps: int = 3u need to turn off them during model evaluation,
    ...
    def train_mode(self):
        ...
        """
    def explain(self, batch_q_doc_vectors):
        ...
        a3 = a1 * a2
        a = batch_q_doc_vectors.reshape(a3, 46)

        M_explain, masks = self.sf.forward_masks(a)
  ...
  # switch evaluation mode
   ...
   # switch evaluation mode
        ...
        for batch_ids, batch_q_doc_vectors, batch_std_labels in test_data:  # batch_size, [batch_size, num_docs, num_features], [batch_size, num_docs]
            ...
            batch_preds = self.predict(batch_q_doc_vectors)
            if self.gpu: batch_preds = batch_preds.cpu()
            x = batch_std_labels.shape
            batch_preds = batch_preds.reshape(x)
            _, batch_pred_desc_inds = torch.sort(batch_preds, dim=1, descending=True)
            batch_predict_rankings = torch.gather(batch_std_labels, dim=1, index=batch_pred_desc_inds)
            if presort:
                batch_ideal_rankings = batch_std_labels
    ...
    def _compute_feature_importances(self, loader):
        ...
        """
        self.sf.eval()
        feature_importances_ = np.zeros((self.sf.post_embed_dim))
...
class RankNet(TabNetRanker):
    ...
    def custom_loss_function(self, batch_preds, batch_std_labels, **kwargs):
        ...
        #a=torch.unsqueeze(batch_preds, dim=2)
        x= batch_std_labels.shape
        batch_preds=batch_preds.reshape(x)
        #print(batch_preds.shape)

        #b = torch.unsqueeze(batch_preds, dim=1)
        #print(b[0])
...
def evaluation(data_id=None, dir_data=None, model_id=None, batch_size=100):
    ...
    for fold_k in range(1, fold_num + 1):   # evaluation over k-fold data
        ...
        for epoch_k in range(1, epochs + 1):
            torch_fold_k_epoch_k_loss = ranker.train(train_data, epoch_k=epoch_k, presort=True)
            """
            ...
            performance_list = [' epoch-' +str(epoch_k)+model_id + ' Fold-' + str(fold_k)]  # fold-wise performance
        ...
        for i, co in enumerate(cutoffs):
            performance_list.append('nDCG@{}:{:.4f}'.format(co, fold_ndcg_ks[i]))
        performance_str = '\t'.join(performance_list)
        #ranker._compute_feature_importances(train_data)
        #for batch_ids, batch_q_doc_vectors, batch_std_labels in test_data:  # batch_size, [batch_size, num_docs, num_features], [batch_size, num_docs]
          #explain_matrix, masks = ranker.explain(batch_q_doc_vectors)
        ...
        plt.savefig("../picture/"+str(performance_list1)+".jpg")
    ...
    # print(ranker._compute_feature_importances(train_data))
